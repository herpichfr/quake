...
from quake.menus import mk_terminal_context_menu
from quake.menus import mk_tab_context_menu
from quake.globals import PCRE2_MULTILINE
from quake.dialogs import RenameDialog
from quake.dialogs import PromptResetColorsDialog
from quake.callbacks import TerminalContextMenuCallbacks
...
class RootTerminalBox(Gtk.Overlay, TerminalHolder):
    def __init__(self, quake, parent_notebook):
        super().__init__()
        self.quake = quake
    ...
    def _add_search_box(self):
        ...
        self.search_entry.connect(
            "key-press-event", self.on_search_entry_keypress)
        self.search_entry.connect("changed", self.set_search)
        self.search_entry.connect("activate", self.do_search)
        self.search_entry.connect(
            "focus-in-event", self.on_search_entry_focus_in)
        self.search_entry.connect(
            "focus-out-event", self.on_search_entry_focus_out)
        ...
        self.search_frame.connect(
            "unmap", lambda x: self.search_revealer.hide())
    ...
    def get_window(self):
        return self.quake.window

    def get_settings(self):
        return self.quake.settings
    ...
    def save_box_layout(self, box, panes: list):
        ...
        if isinstance(box, DualTerminalBox):
            btype = "dual" + \
                ("_h" if box.orient is DualTerminalBox.ORIENT_V else "_v")
    ...
    def restore_box_layout(self, box, panes: list):
        ...
        if cur["type"].startswith("dual"):
            while True:
                if self.quake:
                    # If Quake are not visible, we should pending the restore, then do the
                    # restore when Quake is visible again.
                    #
                    # Otherwise we will stuck in the infinite loop, since new DualTerminalBox
                    # cannot get any allocation when Quake is invisible
                    if (
                        not self.quake.window.get_property("visible")
                        or self.get_notebook()
                        is not self.quake.notebook_manager.get_current_notebook()
                    ):
                        panes.insert(0, cur)
                        self.quake._failed_restore_page_split.append(
                            (self, box, panes))
                        return
    ...
    def show_search_box(self):
        if not self.search_revealer.get_reveal_child():
            GObject.signal_handler_block(
                self.search_revealer, self.search_revealer_show_cb_id)
            self.search_revealer.set_visible(True)
            self.search_revealer.set_reveal_child(True)
            GObject.signal_handler_unblock(
                self.search_revealer, self.search_revealer_show_cb_id)
            # XXX: Mestery line to avoid Gtk-CRITICAL stuff
            # (quake:22694): Gtk-CRITICAL **: 18:04:57.345:
...
class TerminalBox(Gtk.Box, TerminalHolder):
    ...
    def set_terminal(self, terminal):
        ...
        self.terminal.handler_ids.append(
            self.terminal.connect("button-press-event",
    ...
    def get_quake(self):
        return self.get_parent().get_quake()
...
class TabLabelEventBox(Gtk.EventBox):
    def __init__(self, notebook, text, settings):
        ...
        self.box = Gtk.Box(
            homogeneous=Gtk.Orientation.HORIZONTAL, spacing=0, visible=True)
    ...
    def grab_focus_on_last_focused_terminal(self):
        server_time = get_server_time(self.notebook.quake.window)
        self.notebook.quake.window.get_window().focus(server_time)
    ...
    def on_button_press(self, target, event, user_data):
        if event.button == 3:
            ...
            return True
        if event.button == 2:
            prompt_cfg = self.notebook.quake.settings.general.get_int(
                "prompt-on-close-tab")
    ...
    def on_rename(self, user_data):
        HidePrevention(self.get_toplevel()).prevent()
        dialog = RenameDialog(self.notebook.quake.window,
                              self.label.get_text())
        r = dialog.run()
        if r == Gtk.ResponseType.ACCEPT:
            new_text = TabNameUtils.shorten(
                dialog.get_text(), self.notebook.quake.settings)
    ...
    def on_reset_custom_colors(self, user_data):
        HidePrevention(self.get_toplevel()).prevent()
        if PromptResetColorsDialog(self.notebook.quake.window).reset_tab_custom_colors():
            page_num = self.notebook.find_tab_index_by_label(self)
            for t in self.notebook.get_nth_page(page_num).iter_terminals():
                t.reset_custom_colors()
            self.notebook.quake.set_colors_from_settings_on_page(
                page_num=page_num)
        HidePrevention(self.get_toplevel()).allow()
